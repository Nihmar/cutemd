...
class PreviewWebEngineView(QWebEngineView):
    ...
    _SCROLL_LISTENER_JS = (
        ...
        "if(window._cutemd_suppress&&Date.now()<window._cutemd_suppress){"
        "console.log('cutemd: scroll IGNORED suppress='+window._cutemd_suppress+' now='+Date.now());"
        "return;"
        "}"
        ...
        "console.log('cutemd: scroll listener set _cutemd_line='+best+' scrollY='+window.scrollY+' scrollH='+document.body.scrollHeight);"
        "_last_reported=best;"
        "}"
    ...
    _KATEX_JS = (
        ...
        "var sy0=window.scrollY,sh0=document.body.scrollHeight;"
        "window._cutemd_suppress=Date.now()+2000;"
        "console.log('cutemd: katex batch '+entries.length+' el, scrollY='+sy0+' scrollH='+sh0);"
        ...
        "console.log('cutemd: katex rendered, scrollY='+window.scrollY+' scrollH='+document.body.scrollHeight+' suppress='+window._cutemd_suppress+' now='+Date.now());"
        "}catch(e){console.warn('kaTeX error:',e);}"
        "observer.unobserve(el);"
        "});"
        "var sy1=window.scrollY,sh1=document.body.scrollHeight;"
        "console.log('cutemd: katex done, scrollY='+sy0+'->'+sy1+' scrollH='+sh0+'->'+sh1);"
        "},{rootMargin:'300px'});"
        "document.querySelectorAll('.math-katex[data-latex]').forEach(function(el){"
        "observer.observe(el);"
        "});"
