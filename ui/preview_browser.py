...
class PreviewWebEngineView(QWebEngineView):
    ...
    _KATEX_JS = (
        ...
        "var queue=[];"
        "entries.forEach(function(e){if(e.isIntersecting)queue.push(e.target);});"
        "if(!queue.length)return;"
        "window._cutemd_suppress=Date.now()+2000;"
        "var sy0=window.scrollY,sh0=document.body.scrollHeight;"
        "console.log('cutemd: katex queue '+queue.length+' el, scrollY='+sy0+' scrollH='+sh0);"
        "function processNext(){"
        "if(!queue.length){"
        "console.log('cutemd: katex batch done, scrollY='+sy0+'->'+window.scrollY+' scrollH='+sh0+'->'+document.body.scrollHeight);"
        "return;"
        "}"
        "var el=queue.shift();"
        "var latex=el.getAttribute('data-latex');"
        "if(!latex){processNext();return;}"
        "var display=el.classList.contains('math-block');"
        "try{"
        "katex.render(latex,el,{displayMode:display,throwOnError:false});"
        "}catch(ex){console.warn('kaTeX error:',ex);}"
